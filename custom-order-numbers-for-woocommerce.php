...
if ( 'custom-order-numbers-for-woocommerce.php' === basename( __FILE__ ) ) {
	...
	$plugin = 'custom-order-numbers-for-woocommerce-pro.php/custom-order-numbers-for-woocommerce-pro.php';
	if (
		in_array( $plugin, apply_filters( 'active_plugins', get_option( 'active_plugins', array() ) ) ) ||
		( is_multisite() && array_key_exists( $plugin, get_site_option( 'active_sitewide_plugins', array() ) ) )
	) {
...
final class Alg_WC_Custom_Order_Numbers {
	...
	function action_links( $links ) {
		...
		if ( 'custom-order-numbers-for-woocommerce.php' === basename( __FILE__ ) ) {
			$custom_links[] = '<a href="http://coder.fm/item/custom-order-numbers-for-woocommerce/">' . __( 'Unlock All', 'custom-order-numbers-for-woocommerce' ) . '</a>';
		}
